<x-admin-layout>
    <div class="container mx-auto px-4 py-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-gray-900">{{ __('Ajouter une FAQ') }}</h1>
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden p-6">
            <form action="{{ route('admin.faq.store', ['locale' => app()->getLocale()]) }}" method="POST">
                @csrf
                ...
                <div class="mb-8">
                    ...
                    <div class="mb-6">
                        <div class="flex items-center">
                            ...
                            <input type="checkbox" name="is_active" id="is_active" value="1" class="mr-2" {{ old('is_active', '1') == '1' ? 'checked' : '' }}>
                            ...
                            <div x-show="activeTab === '{{ $language->id }}'" class="border rounded-lg p-4 bg-gray-50">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" 
                                           ...
                                           value="{{ old('translations.' . $language->id . '.question') }}" 
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea name="translations[{{ $language->id }}][answer]" 
                                              rows="5" 
                                              class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                              @if($language->is_default) required @endif>{{ old('translations.' . $language->id . '.answer') }}</textarea>
                                </div>
                            </div>
                        @endforeach
                    </div>

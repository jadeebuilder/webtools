<x-admin-layout>
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden p-6">
            <form action="{{ route('admin.faq_categories.update', ['locale' => app()->getLocale(), 'faq_category' => $faqCategory->id]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-8">
                    ...
                    <div class="mb-6">
                        <div class="flex items-center">
                            <input type="checkbox" name="is_active" id="is_active" class="mr-2" {{ old('is_active', $faqCategory->is_active) ? 'checked' : '' }}>
                            ...
                            <div x-show="activeTab === '{{ $language->id }}'" class="border rounded-lg p-4 bg-gray-50">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" 
                                           ...
                                           value="{{ old('translations.' . $language->id . '.name', isset($translations[$language->id]) ? $translations[$language->id]->name : '') }}" 
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea name="translations[{{ $language->id }}][description]" 
                                              rows="3" 
                                              class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('translations.' . $language->id . '.description', isset($translations[$language->id]) ? $translations[$language->id]->description : '') }}</textarea>
                                </div>
                            </div>
                        @endforeach
                    </div>
